...
- **`_surfaces.scss`**, **`_inputs.scss`**, **`_buttons.scss`**, **`_chrome.scss`**, **`_header-search.scss`** (top bar search input + leading icon control), **`_modals.scss`** (dialogs / e.g. Widgets “New Widget”), **`_sidebar.scss`**, **`_text-tokens.scss`** — UI overrides.
